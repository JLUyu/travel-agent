...
For `agent`, custom skills should live under `skills/` directory so they can be discovered automatically at runtime (for example, `./skills/my-skill/SKILL.md`).
...
Keep frontmatter minimal. In `agent`, `metadata` and `always` are also supported when needed, but avoid adding extra fields unless they are actually required.
