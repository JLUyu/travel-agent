...
class TravelAgent:
    ...
    def _merge_command_update(self, state: AgentState, command: Command) -> AgentState:
        ...
        for key, value in update.items():
            ...
            elif key in {"tool_calls", "tool_call_chain", "final_response", "retrieval_context", "user_input"}:
                state[key] = value
            else:
                state[key] = value
        return state
    ...
    def _build_forced_web_search_call(self, state: AgentState) -> dict | None:
        ...
        # Prefer the raw request captured at the HTTP/stream boundary.  Session
        # memory may compact or reshape ``messages`` before the graph runs, so
        # relying exclusively on the reconstructed context can miss the current
        # request in production even though it is visible in the UI.
